...
%CURRENTLY SET UP TO RUN FOR 22kW charge point install
...
DCcost=720; %cost of one dockChain in £
CPcost = 20000; %cost of 22kW chargepoint installation in £
max_time = 8.5*60; 

spaces = 100; %number of spaces in the car park
budget = 100000;
...
for MC =1:sim_runs %632 seconds for 20 runs
    ...
    demand = time(store); %convert desired distances to times
...
%             if  (((x_1*CPcost)+(x_2*DCcost)) <= budget) && (s <= spaces)
%             %constraints for spaces and budget (if statement not used due to mesh requirement on surface plot, surface constraints used instead)
...
% divide by number of simulation runs
...
xPlane2 = [0 5 5 0];      % X coordinates of plane corners, ordered around the plane
...
saveas(s1, 'MCplot22kW.png') %change name so you don't overwrite the wrong .jpg
...
%% Timing function
 ...
 const = 2.5;
 chargerspeed = 22; %7kW charger %change to 22 for 22kW charger
 ...
 % costs
DCcost = 720; %cost of one dockChain in £
CPcost = 20000; %cost of 7kW chargepoint installation in £
...
revenue = Q*E_S*22 + fee_per_fullcharge*satisfied; %change from *7 to *22 for 22kW charger
var_cost = (Q*E_B*22);%change from *7 to *22 for 22kW charger
